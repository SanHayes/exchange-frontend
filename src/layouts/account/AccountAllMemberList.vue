  ...
  <div id="list-all-account" class="data-list-container">
            ...
            <vs-tr :data="tr" :key="tr.id" v-for="(tr, indextr) in data">
              <vs-td>
                ...
                <p>
                  ...
                  <br />
                  *** Tổng CHƠI: <span class="italic" style="color: #ff9900">${{ formatPrice(tr.pricePlay, 2) }}</span
                  ><br />
                  *** Tổng THẮNG: <span class="italic" style="color: #10ff00">${{ formatPrice(tr.priceWin, 2) }}</span
                  ><br />
                  *** Tổng THUA: <span class="italic" style="color: #ff5c5c">${{ formatPrice(tr.priceLose, 2) }}</span
                  ><br />
                  *** Lời: <span class="italic font-bold" style="color: #00ff89">${{ formatPrice(tr.priceWin - tr.priceLose, 2) }}</span
                  ><br />
              ...
              <vs-td>
                ...
                <p>
                  ...
                  <vs-button style="margin: 5px 0" @click="showTKLive(tr.ref_code)"> Số dư tài khoản </vs-button>
                  <vs-button color="warning" @click="showChart(tr.email)"> Hồ sơ nạp tiền </vs-button>
                </p>
              </vs-td>
    ...
    <vs-popup title="Danh sách thành viên F1 - F7" :active.sync="popupF1F7">
      <div id="loading-corners2" class="vs-con-loading__container">
        <p>
          <span class="mr-2">Khối lượng GH tháng này: ${{ formatPrice(tslgdCD1, 2) }}</span
          ><br />
          <span class="mr-2">Khối lượng GH tháng trước: ${{ formatPrice(tslgdCD2, 2) }}</span
          ><br />
          <span class="mr-2">Khối lượng GH 2 tháng trước: ${{ formatPrice(tslgdCD3, 2) }}</span
          ><br />
          <span class="mr-2">Khối lượng GH 3 tháng trước: ${{ formatPrice(tslgdCD4, 2) }}</span
          ><br />
        </p>
    ...
    <vs-popup title="Hồ sơ nạp tiền" :active.sync="popupAddMoney"
    class="popup-history"
    >
      <vs-table ref="table" :data="historyList">
        <template slot="thead">
        ...
        <template slot-scope="{ data }">
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="de-amount">{{ getAmountDecimal(tr.currency, tr.amount) }}<br /></p>
              </vs-td>
              <vs-td>
...
`;
    ...
    // 查看充值记录
    showChart(email) {
      this.popupAddMoney = true;
      AuthenticationService.getDepositListHistory().then((res) => {
        if (res.data.success === 1) {
          this.historyList = res.data.data.filter((v) => v.email === email);
        }
      });
    },
    activeAccount(tr) {
      ...
      AuthenticationService.activeUser(data)
        .then((req) => {
    ...
    turnOff2fa(tr) {
      AuthenticationService.adminDisable2fa({ email: tr.email }).then((resp) => {
    ...
    showCapTren(ref) {
      ...
      AuthenticationService.getSuperior(ref).then((resp) => {
...
.popup-history.con-vs-popup .vs-popup{
  width: 1000px;
}
</style>
